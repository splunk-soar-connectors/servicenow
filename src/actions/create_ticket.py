...
class CreateTicketOutput(PermissiveActionOutput):
    ...
    opened_at: str | None = OutputField(example_values=["2018-11-22 09:57:05"])
    order: str | None = None
    ...
    priority: str | None = OutputField(example_values=["5"])
...
@app.view_handler(template="servicenow_create_ticket.html")
def create_ticket_view(outputs: list[CreateTicketOutput]) -> dict:
    """Transform create ticket action results for HTML rendering."""
    results = []
...
@app.action(
    ...
    view_handler=create_ticket_view,
)
def create_ticket(
